...
export default function Story({ onDone }: { onDone: () => void }) {
  ...
  const speakers: Record<string, Speaker> = {
    minnos: {
      name: t("Kasiyer Minnoş", "Cashier Cutie"),
      bubble: "bg-lavender",
      side: "left",
    ...
    robo: {
      name: t("Müdür Robo", "Manager Robo"),
      bubble: "bg-sky",
      side: "right",
  ...
  const lines: DialogueLine[] = [
    ...
    },
    {
      ...
      visual: (
        <div className="flex flex-wrap gap-2">
          <Pack emoji="🥚" name={t("düzine", "dozen")} n="12" unit={unit} color="bg-peach" />
          {lang === "tr" ? <Pack emoji="🌹" name="deste" n="10" unit={unit} color="bg-pink" /> : <Pack emoji="🧦" name="pair" n="2" unit={unit} color="bg-pink" />}
          <Pack emoji="⚛️" name="mol" n={t("6,02·10²³", "6.02×10²³")} unit={t("tane", "particles")} color="bg-lemon" />
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      visual: (
        <Formula color="bg-lemon">
          1 mol = N<sub>A</sub> = {t("6,02·10²³ tanecik", "6.02×10²³ particles")}
        </Formula>
      ),
    },
    {
    ...
    },
    {
      ...
      visual: <Formula color="bg-sky">{t("M(H₂O) = 2·1 + 1·16 = 18 g/mol", "M(H₂O) = 2×1 + 1×16 = 18 g/mol")}</Formula>,
    },
    {
      speaker: "robo",
      mood: "thinking",
    ...
    },
    {
      ...
      visual: <Formula color="bg-lavender">{t("NK'da: V = n · 22,4 L", "At STP: V = n × 22.4 L")}</Formula>,
    },
    {
      speaker: "minnos",
      mood: "love",
  ...
  return <Dialogue speakers={speakers} lines={lines} onDone={onDone} doneLabel={t("Kasaya geç! 🛒", "To the till! 🛒")} />;
}
